...
function loadState(){
  ...
  try {
    ...
    if(saved.version === initialData.version){
      ...
      saved.reservations?.forEach(r=>{if(r.scheduleId===undefined&&reservationLinks[r.id]&&saved.schedules.some(s=>s.id===reservationLinks[r.id]))r.scheduleId=reservationLinks[r.id]});
    ...
    const reservationLinks={r2:'s2',r3:'s3',r4:'s9',r5:'s10'};
    if(Array.isArray(saved.reservations)) migrated.reservations=saved.reservations.map(r=>({...r,scheduleId:r.scheduleId??(saved.schedules?.some(s=>s.id===reservationLinks[r.id])?reservationLinks[r.id]:''),people:MEMBERS.length,booker:memberMap[r.booker]||MASTER}));
...
function navigate(page){
  ...
  const names={home:['TRIP OVERVIEW','여행 대시보드'],schedule:['DAY BY DAY','날짜별 일정'],foliage:['AUTUMN WATCH','단풍 스팟'],food:['FOOD SHORTLIST','맛집 DB'],drinks:['NIGHT ROUTES','술 대시보드'],map:['ALL PLACES','장소'],budget:['TRIP WALLET','경비'],booking:['RESERVATION BOARD','예약 관리'],checklist:['READY TO GO','출발 전 체크리스트']};
...
function renderStats(){
  ...
  const planned=tripSchedules.reduce((a,b)=>a+(Number(b.cost)||0)*(b.costBasis==='total'?1:MEMBERS.length),0);
...
function renderTimeline(){
  ...
  const firstDay=TRIP_DATES.find(d=>d.date===activeDay)||TRIP_DATES[0],route=state.schedules.filter(item=>item.date===firstDay?.date&&item.time>='17:00').sort((a,b)=>a.time.localeCompare(b.time)).slice(0,4);
  document.querySelector('#homeNightTitle').textContent=firstDay?`${firstDay.short} · ${firstDay.theme}`:'첫날 저녁 일정';
  document.querySelector('#nightRoute').innerHTML=route.length?route.map((item,i)=>`<button type="button" class="route-stop" data-linked-schedule="${item.id}"><i>${i+1}</i><b>${esc(item.place)}</b><small>${item.time}</small></button>`).join(''):'<p class="empty-state">첫날 저녁 일정을 추가해보세요.</p>';
...
function renderFood(){
  ...
  document.querySelector('#foodDatabase').innerHTML=`<div class="db-row header"><span>가게 / 대표 메뉴</span><span>지역</span><span>가격대</span><span>방문일</span><span>우선순위</span><span>별점</span><span>투표</span><span></span></div>`+list.map(p=>`<div class="db-row"><div class="db-name"><b>${esc(p.name)}</b><small>${esc(p.menu)} · ${esc(p.hours)}</small></div><span>${esc(p.area)}</span><span>${esc(p.price)}</span><span>${esc(state.schedules.filter(s=>s.placeId===p.id&&s.placeSource==='food').map(s=>s.date.slice(5)).join(', ')||p.visit)}</span><label class="food-priority-control"><i class="priority-dot ${p.priority}"></i><select data-food-priority="${p.id}" aria-label="${esc(p.name)} 우선순위"><option value="must" ${p.priority==='must'?'selected':''}>무조건 가기</option><option value="maybe" ${p.priority==='maybe'?'selected':''}>시간 되면</option><option value="candidate" ${p.priority==='candidate'?'selected':''}>후보</option></select></label><span class="stars">${'★'.repeat(p.stars)}${'☆'.repeat(5-p.stars)}</span><button class="vote-button ${p.voted?'voted':''}" data-vote="food:${p.id}">👍 ${p.votes}</button><button class="icon-button" data-edit-place="food:${p.id}">›</button><button type="button" class="secondary" data-add-visit="food:${p.id}">일정에 추가</button></div>`).join('');
}
function renderDrinkRoutes(){
  const routes=TRIP_DATES.map(day=>({day,stops:state.schedules.filter(item=>item.date===day.date&&item.time>='17:00'&&['food','drink','hotel'].includes(item.category)).sort((a,b)=>a.time.localeCompare(b.time))})).filter(route=>route.day.date===activeDay);
...
function renderMapsImportReview(){
  ...
  document.querySelector('#mapsImportSummary').innerHTML=`<b>홋카이도 ${counts.hokkaido}개</b><span>타지역 제외 ${counts.outside}개 · 직접 확인 ${counts.unknown}개 · 선택 ${selected}개</span>`;
  document.querySelector('#mapsImportList').innerHTML=pendingMapsImports.map((place,index)=>`<article class="maps-import-row ${place.region}"><input type="checkbox" data-import-select="${index}" ${place.selected?'checked':''} ${place.region==='outside'?'disabled':''} aria-label="${esc(place.name)} ${place.region==='unknown'?'홋카이도로 직접 확인':'가져오기 선택'}"><div><b>${esc(place.name)}</b><small>${esc(place.address||place.sourceList)} · ${place.region==='hokkaido'?'홋카이도':place.region==='outside'?'타지역 제외':'지도 확인 후 직접 선택'}</small></div><select data-import-category="${index}">${['food','drink','cafe','hotel','shop','tour','move'].map(category=>`<option value="${category}" ${place.category===category?'selected':''}>${CAT[category]?.label||category}</option>`).join('')}</select><a href="${place.url}" target="_blank" rel="noopener" aria-label="Google Maps에서 확인">↗</a></article>`).join('')||'<p class="empty-state">읽을 수 있는 장소가 없습니다. CSV 또는 JSON 내용을 확인해주세요.</p>';
  document.querySelector('#confirmMapsImport').disabled=!selected;
  document.querySelector('#verifyMapsRegions').disabled=true;
}
...
const validCoordinates=place=>place.lat!==null&&place.lng!==null&&place.lat!==''&&place.lng!==''&&Number.isFinite(Number(place.lat))&&Number.isFinite(Number(place.lng))&&Math.abs(Number(place.lat))<=90&&Math.abs(Number(place.lng))<=180;
...
function runConsistencyChecks(){
  ...
  const tests={
    ...
    uniqueExpenseIds:new Set(state.expenses.map(e=>e.id)).size===state.expenses.length,
...
function placeStayMinutes(value,fallback=90){const number=Number.parseFloat(value);return Number.isFinite(number)?Math.min(1439,Math.max(0,Math.round(number*(String(value).includes('시간')?60:1)))):fallback}
function scheduleSourceOptions(){
  return [
    ...
    ...state.foliage.map(item=>({label:`단풍 · ${item.name} · ${item.area}`,item,category:'tour',description:item.note||item.fit||'',duration:placeStayMinutes(item.duration),transport:item.transport||'확인 필요',reservation:'불필요',memo:[item.season,item.status,item.weather].filter(Boolean).join(' · ')})),
...
function openScheduleEditor(id,draft={}){
  const item=id?state.schedules.find(s=>s.id===id):{id:uid('s'),date:activeDay,time:'12:00',end:'13:00',place:'',category:'tour',description:'',duration:60,nextTravel:0,transport:'도보',cost:0,reservation:'확인 필요',reservationTime:'',map:'',official:'',memo:'',...draft};
  ...
  const fillFromSource=source=>{if(!source)return;const sourceItem=source.item,fields={place:sourceItem.name,category:source.category,description:source.description,duration:source.duration,nextTravel:15,transport:source.transport,cost:0,reservation:source.reservation,map:sourceItem.map||'',official:sourceItem.official||sourceItem.booking||'',memo:source.memo};if(form.elements.placeId){const p=allMapPlaces().find(p=>p.id===sourceItem.id&&mapPlaceSourceCollection(p.mapSource)?.includes(sourceItem));form.elements.placeId.value=p?.id||'';form.elements.placeSource.value=p?.mapSource||'';form.elements.place.readOnly=!!p;form.elements.map.readOnly=!!p}Object.entries(fields).forEach(([key,value])=>{if(form.elements[key]&&(!id||!['duration','nextTravel','cost','memo','reservation'].includes(key)))form.elements[key].value=value??''});if(!id&&form.elements.time){const start=parseSmartTime(form.elements.time.value);if(start){const total=Number(start.slice(0,2))*60+Number(start.slice(3))+Number(form.elements.duration.value);form.elements.end.value=`${String(Math.floor(total/60)%24).padStart(2,'0')}:${String(total%60).padStart(2,'0')}`}}sourceSearch.value=sourceItem.name;results.innerHTML='';toast(`${sourceItem.name} 정보를 일정에 불러왔어요.`)};
...
function openMapPlaceEditor(id){
  const place=allMapPlaces().find(item=>item.id===id);if(!place)return;
  if(place.mapSource==='foliage'){openFoliageEditor(id);return}
  const original=mapPlaceSourceCollection(place.mapSource)?.find(item=>item.id===id);if(!original)return;
  editing={type:'mapPlace',id,isNew:false,source:place.mapSource,item:original};document.querySelector('#modalEyebrow').textContent='MAP PLACE';document.querySelector('#modalTitle').textContent='장소 관리';document.querySelector('#deleteItem').style.visibility='visible';
  const categories=['food','drink','cafe','hotel','shop','tour','move'];
...
function saveMapPlace(values,persist=true){
  ...
  const target=values.mapCategory,lat=values.lat===''?null:Number(values.lat),lng=values.lng===''?null:Number(values.lng),coordinatesVerified=Number.isFinite(lat)&&Number.isFinite(lng)&&Math.abs(lat)<=90&&Math.abs(lng)<=180,common={id:original.id,name:values.name.trim(),area:values.area.trim()||'홋카이도',address:values.address.trim(),map:values.map.trim(),lat:coordinatesVerified?lat:null,lng:coordinatesVerified?lng:null,coordinatesVerified,sourceList:original.sourceList||'',votes:Number(original.votes)||0,voted:Boolean(original.voted)};
  let updated;
  if(target==='food')updated={...original,...common,type:'food',menu:editing.source==='food'?(original.menu||'메뉴 확인 필요'):'메뉴 확인 필요',category:editing.source==='food'?(original.category||'미분류'):'미분류',price:original.price||'확인 필요',hours:original.hours||'확인 필요',closed:original.closed||'확인 필요',reservable:original.reservable||'확인 필요',reserved:Boolean(original.reserved),wait:original.wait||'확인 필요',rating:original.rating||'확인 필요',review:original.review||'',booking:original.booking||'',visit:original.visit||'미정',priority:['must','maybe','candidate'].includes(original.priority)?original.priority:'candidate',stars:Number.isFinite(Number(original.stars))?Number(original.stars):3,memo:values.note.trim()};
  else if(target==='drink')updated={...original,...common,type:'drink',mood:original.mood||'확인 필요',alcohol:original.alcohol||'확인 필요',menu:original.menu||'메뉴 확인 필요',category:editing.source==='drink'?(original.category||'미분류'):'미분류',price:original.price||'확인 필요',hours:original.hours||'확인 필요',lastOrder:original.lastOrder||'확인 필요',reservable:original.reservable||'확인 필요',smoking:original.smoking||'확인 필요',crowd:original.crowd||'확인 필요',booking:original.booking||'',priority:Number(original.priority)||3,stage:original.stage||'후보',note:values.note.trim()};
  else updated={...original,...common,category:target,url:common.map,note:values.note.trim(),status:original.status||'정보 확인 필요'};
...
document.addEventListener('click',e=>{
  ...
  if(e.target.closest('#verifyMapsRegions')){toast('요금 0원 모드에서는 Google Places API를 호출하지 않아요.');return}
  ...
  const importSelect=e.target.closest('[data-import-select]');if(importSelect){const place=pendingMapsImports[Number(importSelect.dataset.importSelect)];if(place.region==='unknown'&&importSelect.checked){if(!confirm(`${place.name} 장소가 홋카이도에 있는지 Google Maps에서 확인했나요?`)){importSelect.checked=false;return}place.region='hokkaido';place.manualRegion=true}place.selected=importSelect.checked;renderMapsImportReview();return}
...
document.querySelector('#editorForm').addEventListener('submit',e=>{
  ...
  else if(editing.type==='mapPlace')saveMapPlace(values)
  ...
  else if(editing.type==='booking'){const previous=state.schedules.find(s=>s.id===editing.previousScheduleId);if(previous)previous.bookingManaged=true;const item={...editing.item,...values,people:Number(values.people),separateTime:values.separateTimeValue==='yes',followSchedule:true};delete item.separateTimeValue;delete item.scheduleSearch;if(editing.isNew)state.reservations.push(item);else Object.assign(state.reservations.find(r=>r.id===editing.id),item);syncReservationToSchedule(item);saveState(`${item.place} 예약 항목을 ${editing.isNew?'추가':'수정'}했어요`)}
...
document.querySelector('#deleteItem').addEventListener('click',()=>{if(!editing||editing.isNew)return;const arr=editing.type==='mapPlace'?mapPlaceSourceCollection(editing.source):editing.type==='schedule'?state.schedules:editing.type==='food'?state.food:editing.type==='drink'?state.drinks:editing.type==='foliage'?state.foliage:editing.type==='expense'?state.expenses:state.reservations,item=arr?.find(x=>x.id===editing.id),message=editing.type==='mapPlace'?`${item?.name||'이 장소'}을 삭제할까요?\n이미 일정에 추가한 내용은 유지됩니다.`:'이 항목을 삭제할까요?';if(!arr||!confirm(message))return;if(editing.type==='schedule')state.reservations.filter(r=>r.scheduleId===editing.id).forEach(r=>r.scheduleId='');if(editing.type==='booking'){const linked=state.schedules.find(s=>s.id===item?.scheduleId);if(linked)linked.bookingManaged=true}const idx=arr.findIndex(x=>x.id===editing.id);if(idx>=0)arr.splice(idx,1);saveState('항목을 삭제했어요');document.querySelector('#editorDialog').close();renderAll();toast('삭제했어요.');});
