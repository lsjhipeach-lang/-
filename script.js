...
function renderBudget(){
  ...
  document.querySelector('#expenseList').innerHTML=filteredExpenses.map(e=>{const amount=expenseJPY(e);return `<div class="expense-row" data-edit-expense="${e.id}"><small>${e.date.slice(5)}</small><div><b>${esc(e.description)}</b><small>${e.category} · 결제 ${e.payer}${e.scope==='personal'?` · 사용 ${e.owner}`:' · 공동'}</small></div><div class="amount"><b>${won(amount)}</b><small>${yen(amount)}${e.inputCurrency==='KRW'?' · 원화 입력':''}</small></div><span class="expense-scope ${e.scope}">${e.scope==='personal'?'개인':'공동'}</span><span class="tag ${e.settled?'done':'need'}">${e.settled?'정산 완료':'미정산'}</span></div>`}).join('')||'<p class="empty-state expense-filter-empty">조건에 맞는 지출이 없습니다.</p>';
...
function openScheduleEditor(id){
  ...
  document.querySelector('#editorFields').innerHTML=`<label class="field wide dashboard-source-picker"><span>대시보드 정보 불러오기</span><input id="scheduleSourceSearch" list="scheduleSourceOptions" placeholder="맛집, 단풍, 술집 또는 저장 장소 검색" autocomplete="off"><datalist id="scheduleSourceOptions">${sources.map(source=>`<option value="${esc(source.label)}"></option>`).join('')}</datalist><small>항목을 선택하면 아래 일정 정보가 자동 입력되며 필요한 내용만 수정할 수 있어요.</small></label><label class="field"><span>날짜</span><select name="date">${TRIP_DATES.map(d=>`<option value="${d.date}" ${d.date===item.date?'selected':''}>${d.short} ${d.weekday}</option>`).join('')}</select></label>`+scheduleFields.map(([key,label,type,wide])=>{if(type==='select'){const opts=key==='category'?Object.entries(CAT).map(([v,c])=>`<option value="${v}" ${v===item[key]?'selected':''}>${c.label}</option>`).join(''):['조사 필요','확인 필요','예약 예정','예약 요청','예약 완료','불필요'].map(v=>`<option ${v===item[key]?'selected':''}>${v}</option>`).join('');return `<label class="field ${wide||''}"><span>${label}</span><select name="${key}">${opts}</select></label>`}return `<label class="field ${wide||''}"><span>${label}</span>${type==='textarea'?`<textarea name="${key}">${esc(item[key])}</textarea>`:`<input type="${type}" name="${key}" value="${esc(item[key])}">`}</label>`}).join('');
  const form=document.querySelector('#editorForm'),sourceSearch=document.querySelector('#scheduleSourceSearch');
  const fillFromSource=()=>{const source=sources.find(option=>option.label===sourceSearch.value);if(!source||sourceSearch.dataset.loaded===source.label)return;sourceSearch.dataset.loaded=source.label;const sourceItem=source.item,fields={place:sourceItem.name,category:source.category,description:source.description,duration:source.duration,nextTravel:15,transport:source.transport,cost:0,reservation:source.reservation,map:sourceItem.map||'',official:sourceItem.official||sourceItem.booking||'',memo:source.memo};Object.entries(fields).forEach(([key,value])=>{if(form.elements[key])form.elements[key].value=value??''});toast(`${sourceItem.name} 정보를 일정에 불러왔어요.`)};
  sourceSearch.addEventListener('input',fillFromSource);sourceSearch.addEventListener('change',fillFromSource);
